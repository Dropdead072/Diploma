...
Главный смысл Квазиньютонных методов - приближение Гессиана (или сразу обратного Гессиана $B_k$), причем $B_k$ стремится к значению обратного Гессиана в точке оптимума, если формально:
...
$$
\Delta x_k = B_{k+1} \Delta y_k, \space{} \Delta y_k = \nabla f(x_{k+1}) - \nabla f(x_k)
$$
