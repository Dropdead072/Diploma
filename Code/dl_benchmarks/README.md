...
methods are skipped (memory infeasible). The suite includes:
...
Causal-LM fine-tuning of `Qwen/Qwen2.5-0.5B` on a Russian Wikipedia slice
(or any local text file you provide). **Prints a generation sample from the
*untrained* model before training** so you can compare it against post-training
generations.
